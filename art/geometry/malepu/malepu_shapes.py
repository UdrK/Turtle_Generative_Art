...
def draw_overlapping_triangles(tur, triangle_side, draw_lower_side=False):
    ...
    for i in range(5):
        if i == 0:
            tur.forward_optional_draw(triangle_side, draw_lower_side)
    ...
    tur.setheading(0)
    tur.teleport(halfway_points[0])
    for i in range(4):
        if i == 0 or i == 3:
            tur.forward_optional_draw(triangle_side / 4, draw_lower_side)
        else:
...
def draw_crescent_circle(tur, radius, step_size=3, number_of_crescents=6):
    ...
    circle_center = tur.circle_and_return_center(radius)
    crescents_starting_points = get_points_alongside_circle(
        tur, radius, step_size, number_of_crescents
    )
    i = 1
...
def large_concentric_triangles(tur, side):
    ...
    tur.forward_optional_draw(side * multiplication_factor, False)
    tur.setheading(original_heading)
    draw_triangle(tur, side * 0.95, lambda t, d: t.forward_dashed(d))
...
def draw_star(tur, size):
    ...
    for i in range(spokes):
        ...
        ray_length_multiplicator = calculate_parabola(
            lambda x: (x**2 - x + 1), parabola_x
        )
