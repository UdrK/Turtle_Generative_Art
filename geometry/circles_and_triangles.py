...
CENTER_FULL_CIRCLE_RADIUS = center_FULL_CIRCLE_RADIUS = (
    (BIG_CIRCLE_RADIUS + 1 / 6 * SMALL_CIRCLE_RADIUS) * 0.234
) / 2
...
def draw_circle_centered_on_coordinates(tur, x, y, radius, fill=False):
    ...
    tur.invariant_draw(
        draw_circle_centered_on_coordinates_aux,
...
def draw_two_empty_circles(
    tur, position, direction, big_circle_radius, small_circle_radius, center_full_circle_radius
):
    tur.teleport(position)
    tur.forward_without_drawing(
        small_circle_radius - center_full_circle_radius, direction + 180
    )
...
def draw_big_triangle(tur, position, direction, triangle_side, center_full_circle_radius):
    def draw_vertical_line_with_circles(
        tur, position, direction, triangle_side, center_full_circle_radius
    ):
        ...
        draw_circle_centered_on_coordinates(
            tur, tur.xcor(), tur.ycor(), small_full_circle_radius, True
        )
    ...
    second_segment_position = draw_vertical_line_with_circles(
        tur, position, direction, triangle_side, center_full_circle_radius
    )
    draw_triangle_from_horizontal_side_center(
        tur, tur.position(), tur.heading(), triangle_side
    )
    return second_segment_position
...
def draw_half_drawing(tur, position, direction):
    def draw_small_triangle(tur, triangle_horizontal_side_half_point, triangle_side, direction):
        draw_triangle_from_horizontal_side_center(
            tur, triangle_horizontal_side_half_point, direction, triangle_side
        )

    draw_two_empty_circles(
        tur,
        position,
        direction,
        BIG_CIRCLE_RADIUS,
        SMALL_CIRCLE_RADIUS,
        CENTER_FULL_CIRCLE_RADIUS,
    )
    second_segment_position = draw_big_triangle(
        tur, position, direction, BIG_CIRCLE_RADIUS * 2, CENTER_FULL_CIRCLE_RADIUS
    )
    draw_small_triangle(
        tur,
        second_segment_position,
        (BIG_CIRCLE_RADIUS * 2) * 0.8408,
        direction - 180,
    )
