...
def _default_forward(tur, side):
    tur.forward(side)


def draw_triangle(tur, side, forward_strategy=None):
    if forward_strategy is None:
        forward_strategy = _default_forward
    for _ in range(3):
...
def draw_oval(tur, radius, arc_length):
    ...
    def draw_oval_side(tur, radius, arc_length, direction=Direction.UP):
        def draw_right_half_side(tur, radius, arc_length, direction):
            tur.invariant_draw(
                draw_half_oval_half_side,
                {"radius": radius, "arc_length": direction * arc_length},
            )

        def draw_left_half_side_after_right(tur, radius, arc_length, direction):
            tur.setheading(tur.heading() + 180)
            side_apex = tur.invariant_draw(
                draw_half_oval_half_side,
                {"radius": -1 * radius, "arc_length": direction * arc_length},
            )
            tur.setheading(tur.heading() - 180)
            return side_apex
        ...
        upper_side_starting_point = [
            oval_lower_curve_center[0],
            oval_lower_curve_center[1] + oval_curves_apices_distance * 2,
        ]
        oval_center = [
            oval_lower_curve_center[0],
            oval_lower_curve_center[1] + oval_curves_apices_distance,
        ]
        return [upper_side_starting_point, oval_center, oval_curves_apices_distance]

    [upper_side_center_point, oval_center, oval_height] = draw_oval_side(
        tur, radius, arc_length
    )
    tur.teleport(upper_side_center_point)
    tur.setheading(tur.heading() + 180)
